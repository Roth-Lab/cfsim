...
def run_generate_wig(in_file: str, out_file: str) -> None:
    ...
    df_cfdna = pd.read_csv(in_file, sep="\t").assign(
        start=lambda df: df["start"] + 1, end=lambda df: df["end"] + 1
    )
...
def df_to_wig(df: pd.DataFrame, genome_build: str, output_path: str) -> None:
    ...
    with open(output_path, "w") as f:
        for c in df["chrom"].unique():
            df_c = df.loc[df["chrom"] == c].sort_values("start")
            ...
            s = f"fixedStep chrom={chrom} start={start} step={step} span={step}\n"

            f.write(s)

            # write reads

            f.writelines(f"{int(read)}\n" for read in df_c["reads"].values)
...
def wig_to_df(wig_file: str) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        data={"chrom": chrms, "start": starts, "end": ends, "values": values}
    )
...
def get_chromosome_sizes() -> pd.DataFrame:
    ...
    chroms = [f"chr{c}" for c in range(23)] + ["chrX", "chrY"]
